...
class RegisterViewModel(
    ...
    fun nameState(name: String): InputValid {
        return when {
            name.isEmpty() -> {
                InputValid.Error(R.string.empty_field)
            }

            !name.matches(Regex("^[a-zA-ZÀ-ÿ]+(?: [a-zA-ZÀ-ÿ]+)+\$", RegexOption.IGNORE_CASE)) -> {
                InputValid.Error(R.string.invalid_name)
            }

            name.count { it.isLetter() } < 5 -> {
                InputValid.Error(R.string.at_least_five_letters)
            }
            ...
            when (val response = registerUser(finalUserApi)) {
                ...
                is ApiResponse.ErrorWithDetail -> {
                    ...
                    _apiErrorMessage.value =
                        "Estamos com alguns problemas. Tente novamente mais tarde!"
                }
            }
        }
    }
    ...
    fun passwordState(password: String): PasswordInputValid {
        ...
        passwordHas[R.string.at_least_one_uppercase_letter] =
        ...
        passwordHas[R.string.at_least_one_special_character] =
            Regex("\\W").containsMatchIn(password)
